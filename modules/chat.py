...
def chatbot_wrapper(text, generate_state, name1, name2, context, mode, end_of_turn, regenerate=False, constraint_value="", constraint_toggle=None):
    print(f"toggle value: {constraint_toggle}, schema: {constraint_value}\n\n")
    if mode == 'instruct':
        stopping_strings = [f"\n{name1}", f"\n{name2}"]
    else:
        stopping_strings = [f"\n{name1}:", f"\n{name2}:"]
    ...
    oneof = constraint_value if constraint_toggle == 'one-of' else ''
    json_schema = constraint_value if constraint_toggle == 'json-schema' else ''

    valid_json = constraint_toggle == 'valid-json'
    print(f'valid_json: {valid_json}')

    # Generate
    for i in range(generate_state['chat_generation_attempts']):
        reply = None
        for reply in generate_reply(f"{prompt}{' ' if len(cumulative_reply) > 0 else ''}{cumulative_reply}", generate_state, eos_token=eos_token, stopping_strings=stopping_strings, json_constraint=json_schema, valid_json=valid_json, one_of=oneof):
...
def cai_chatbot_wrapper(text, generate_state, name1, name2, context, mode, end_of_turn, json_schema, constraint_toggle):
    for history in chatbot_wrapper(text, generate_state, name1, name2, context, mode, end_of_turn, constraint_value=json_schema, constraint_toggle=constraint_toggle):
